...
def preprocess_path_for_deform_task(gt_path, distorted_path):
    ...
    for distorted_image in distorted_image_list:
        ...
        if not os.path.isfile(gt_image):
            if not os.path.isfile(gt_image[:-4] + '.png') and not os.path.isfile(gt_image[:-4] + '.jpg'):
                print(distorted_image, gt_image)
                print('=====')
                continue
            elif not os.path.isfile(gt_image[:-4] + '.png'):
                gt_image = gt_image[:-4] + '.jpg'
            else:
                gt_image = gt_image[:-4] + '.png'
        gt_list.append(gt_image)
        distorated_list.append(distorted_image)    

    return gt_list, distorated_list

def preprocess_path_for_deform_task_(gt_path, distorted_path):
    distorted_image_list = sorted(get_image_list(distorted_path))
    gt_list=[]
    distorated_list=[]
    ...
    for distorted_image in distorted_image_list:
        ...
        while True:
            # 查找 _2_ 的位置
            index = s.find('_2_', start_index)
            # 检查 _2_ 后面一个字母是否为 M 或者 W
            if index + 3 < len(s) and (s[index + 3] == 'M' or s[index + 3] == 'W'):
                # 如果满足条件，将 _2_ 之前的部分添加到结果列表
                parts = s[index+3:]
                break
            start_index = index + 3
        image = parts
...
def preprocess_path_for_deform_task_cfld(gt_path, distorted_path):
    distorted_image_list = sorted(get_image_list(distorted_path))
    gt_list=[]
    distorated_list=[]
    ...
    while True:
        ...
        index = s.find('_2_', start_index)
        if index == -1:
            # 如果找不到 _2_ ，将剩余部分添加到结果列表并结束循环
            parts.append(s[start_index:])
            break
        # 检查 _2_ 后面一个字母是否为 M 或者 W
        if index + 3 < len(s) and (s[index + 3] == 'M' or s[index + 3] == 'W'):
            # 如果满足条件，将 _2_ 之前的部分添加到结果列表
            parts.append(s[start_index:index])
            start_index = index + 3
        else:
            # 不满足条件，继续查找下一个 _2_
            start_index = index + 2
...
class MyLPIPS():
    def __init__(self, device, use_gpu=True):
        self.device = device
    ...
    def calculate_from_disk(self, path_1, path_2,img_size, batch_size=64, verbose=False, sort=True):
        ...
        for i in range(n_batches):
            ...
            if self.use_gpu:
                img_1_batch = img_1_batch.to(self.device)
                img_2_batch = img_2_batch.to(self.device)
